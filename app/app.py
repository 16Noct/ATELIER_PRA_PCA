...
def status():
    ...
    backup_dir = "/backup"
    backups = sorted(glob.glob(os.path.join(backup_dir, "app-*.db")))

    if backups:
        last_backup = os.path.basename(backups[-1])
        last_backup_mtime = os.path.getmtime(backups[-1])
        backup_age = round(time.time() - last_backup_mtime)
    else:
        last_backup = None
        backup_age = None
